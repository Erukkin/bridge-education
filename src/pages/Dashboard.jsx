  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '12px' }}>
        <span style={{ background: 'rgba(240,180,41,0.2)', color: 'var(--gold)', fontSize: '11px', fontWeight: 700, padding: '4px 10px', borderRadius: '999px', fontFamily: 'Sora, sans-serif' }}>
          {murid.program}
        </span>
        {isUnassigned ? (
          <span style={{ background: 'rgba(229,62,62,0.25)', color: '#FC8181', fontSize: '11px', fontWeight: 700, padding: '4px 10px', borderRadius: '999px', fontFamily: 'Sora, sans-serif' }}>
            ⚠️ Unassigned
          </span>
        ) : (
  ...
  return (
    ...
    >
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: deleteMode ? '28px' : '12px' }}>
        ...
        <span style={{ background: 'rgba(255,255,255,0.1)', color: 'var(--gray-light)', fontSize: '11px', padding: '4px 10px', borderRadius: '999px' }}>
    ...
    if (viewMode === 'students') {
      if (actionMode === 'delete') {
        ...
        setClasses(classes.map(c =>
          c.name === item.class_id
            ? { ...c, student_ids: c.student_ids.filter(id => id !== item.id) }
            : c
        ))
        setActionMode(null)
      ...
      if (actionMode === 'delete') {
        ...
        const ok = window.confirm(
          `Delete class "${item.name}"?\n\n${affectedCount > 0
            ? `⚠️ ${affectedCount} student(s) will become Unassigned.`
            : 'This class is empty.'
        ...
        if (affectedCount > 0) {
          await Promise.all(
            item.student_ids.map(sid => {
              const s = students.find(x => x.id === sid)
              if (s) return updateStudent(sid, { ...s, class_id: 'Unassigned' })
            })
          )
          setStudents(students.map(s =>
            item.student_ids.includes(s.id) ? { ...s, class_id: 'Unassigned' } : s
          ))
        }
  ...
  async function handleConfirm(form) {
    ...
    if (selectedId) {
      ...
      const configChanged =
        ...
        oldStudent.class_type !== form.classType ||
        oldStudent.age_group !== form.ageGroup ||
      ...
      if (configChanged || oldClassId === 'Unassigned') {
        if (oldClassId !== 'Unassigned') {
          ...
          if (oldKelas) {
            ...
            updatedClasses = updatedClasses.map(c =>
              c.name === oldClassId ? { ...c, student_ids: updatedIds } : c
            )
          }
        }
        ...
        if (isNew) {
          ...
          updatedClasses = updatedClasses.map(c =>
            c.name === classId ? { ...c, student_ids: updatedIds } : c
          )
        }
      }
    ...
    <div style={{ minHeight: '100vh', width: '100%', background: 'var(--navy)', fontFamily: 'DM Sans' }}>
      ...
      <div className="topbar">
        ...
        {viewMode === 'students' && (
          <div className={`search-wrap ${searchFocused ? 'search-focused' : ''}`}>
            ...
            <input
              ...
              placeholder={searchBy === 'name' ? 'Search by name...' : searchBy === 'id' ? 'Search by ID...' : 'Search by class...'}
        ...
        <div className="topbar-actions">
          ...
          {unassignedCount > 0 && (
            <span style={{ background: 'rgba(229,62,62,0.2)', color: '#FC8181', fontSize: '12px', fontWeight: 700, padding: '6px 12px', borderRadius: '8px', border: '1px solid rgba(229,62,62,0.3)', cursor: 'pointer' }}
